...
from common.tools import saveFile,encryption,getHKid
from common.apiCenter import sendSMS,smsCodeLogin
...
def openStatus(args,table_class='personal'):
	url=f'/app/foOpenAccount/openStatus'
	data={
		'token':args.token,
...
def checkTable(args,table_class='personal'):
	url=f'/app/foOpenAccount/checkTable'
	data={
		'token':args.token,
		'table_class':table_class,
		'lang':'cn',
...
def save_first(args,data=None):
	args.invite_code=''
	args.aum_account=''
	url=f'/app/foOpenAccount/save'
	if not data:data={
		...
		'serial_num':f'FromAutoTest_{str(time.time())}',#'dUm5aSaddsX1PV9dKUYqIQTX3Yph1I4p',
	}
	logging.info(f'请求URL: {url}')
	logging.info(f'请求数据: {data}')
	resp=requests.post(url,headers=head,data=data,timeout=30)
...
def checkEmail(args):
	url=f'/app/foOpenAccount/checkEmail'
	data={
		'token':args.token,#'BA31BC9BE6A8D1B66268AE95A7AE924C',
...
def save(args,data=None):
	...
	logging.info(f'请求URL: {url}')
	if args.table_status=='handwrite':
		logging.info(f'请求数据: {data}'.replace(sign_img,'sign_img'))
	resp=requests.post(url,headers=head,data=data,timeout=30)
	respJson=resp.json()
	if args.table_status=='handwrite':
...
def createNotice(args):
	url=f'/app/foOpenAccount/createNotice'
	data={
		'table_no':args.table_no,#'2021081913513131583',
		'table_token':args.table_token,#'7393f12d7731f1650092dc3e72e29b1b',
		'lang':'cn',
...
################################## 二次开户 #################################################################
def secondOpenCheck(args):
	url=f'/app/foOpenAccount/secondOpenCheck'
	data={
		'token':args.token,
		'table_class':'secondPersonal',
		'lang':'cn',
	}
	logging.info(f'请求URL: {url}')
	logging.info(f'请求数据: {data}')
	...
	respJson=resp.json()
	logging.info(f'响应数据: {respJson}')
	# args.ac_cash=respJson['data']['sourceAccount']
	return respJson['result']

def checkSourceAccount(args):
	url=f'/app/foOpenAccount/checkSourceAccount'
	data={
		'token':args.token,
		'sourceAccount':args.acc,
		'lang':'cn',
	}
	logging.info(f'请求URL: {url}')
	logging.info(f'请求数据: {data}')
...
def secondOpenSave(args):
	url=f'/app/foOpenAccount/secondOpenSave'
	data={
		'mk_hk':'1',
		'mk_sha':'1',
		'mk_sza':'1',
		'mk_us':'1',
		'mk_otc':'0',
		'settlement_notify_mail':'1',
		'settlement_notify_postal':'0',
		'is_know_risk':'0',
		'table_status':'start',
		'token':args.token,
		'sourceAccount':args.acc,
	...
	logging.info(f'请求URL: {url}')
	for status in ['start','s_marketchoose','s_riskannounce']:
		data['table_status']=status
		data['serial_num']=f'FromAutoTest_{str(time.time())}'
		if status=='s_marketchoose':
			data['sync_settle_card']='0'
		elif status=='s_riskannounce':
			data['is_know_risk']='1'
			logging.info(f'步骤: {status} 请求数据: {data}')
		resp=requests.post(url,headers=head,data=data,timeout=30)
		respJson=resp.json()
		# logging.info(f'响应数据: {respJson}')
		if status=='start':
			args.table_no=data['table_no']=respJson['data']['table_no']
			args.table_token=data['table_token']=respJson['data']['table_token']
			args.Macc=respJson['data']['secondAccountNo']
	return respJson['result']

def secondOpenComplete(args):
	url=f'/app/foOpenAccount/secondOpenComplete'
	data={
		'token':args.token,
...
def reg_api_second(args):
	sendSMS(args)
	args.token,reason=smsCodeLogin(args)
	# isDefend(args)
	openStatus(args)
	for i in range(30):
		result=secondOpenCheck(args)
		if int(result):
			checkSourceAccount(args)
			secondOpenSave(args)
			secondOpenComplete(args)
			return args
		else:
			if i==9:raise Exception(f'二次开户失败')
			time.sleep(10)

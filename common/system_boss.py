...
def getSMScode(phone,env='uat',n=0):
	...
	for i in range(5):
		...
		resp=requests.get(url,params=param,headers=head,timeout=30)
		try:
			respJson=resp.json()
		except:
			print(resp.content)
...
def openMarket_boss(account,env='uat'):
	...
	if len(respJson['data']['list'])>0:
		...
		key=f'id={checkid}&market[]=SHA&market[]=SZA&market[]=USA&market[]=FUN&market[]=BOND&market[]=SP'
		respJson=requests_boss('POST',path,backType='json',env=env,data=key)
		msg=f'{account} 开通市场结果: {respJson}'
		result.append(msg)
		logging.info(msg)
		return result
...
def getPwd_from_mail(mailid,env='uat',mod='reset'):
	...
	if mod=='reset':# 获取重置的密码
		try:
			...
			pwd=re.findall(r'\d{8}',soup.select('td')[2].text)[0]
...
def addBankcard(account,bankCode,curr='MTC',bankType='238',env='uat'):
	...
	respJson=requests_boss('POST',path,backType='json',env=env,data=key)
	# {'result': '1', 'message': 'success', 'data': {'list': [{'title': '472775 自动化测试后台开户', 'url': '/cms/cmsAccount/check?id=26264'}]}, 'now': '2021-09-01 17:46:13'}
	logging.info(f'新增结算银行卡结果: {respJson}')
...
def getCasherWithdrawal(account,env='uat'):
	...
	for tr in soup.select('tr'):
		try:
			status=tr.select_one("td[class='step_id']").text.strip()
			if status!='完成':
				approveIdList.append([tr.select_one('.business_no').select_one('a')['href'].split('=')[-1],status])
				# approveIdList.append(tr.select('td')[-1].select_one('a')['href'])
